  ...
  public getMaterial() {
    return this.history.moves.reduce<{
      ...
      (acc, move) => {
        ...
        if (!metaWithCapturedPiece?.capturedPieceName) return acc

        const accValue = acc[metaWithCapturedPiece.movedPieceColor]
        accValue.material += metaWithCapturedPiece.capturedPieceValue ?? 0
        accValue.pieceNames.push(metaWithCapturedPiece.capturedPieceName)
        return acc
      },
      {
        [Colors.WHITE]: { material: 0, pieceNames: [] },
        [Colors.BLACK]: { material: 0, pieceNames: [] },
      },
    )
  }
}
